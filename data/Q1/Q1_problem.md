...
**C.2 逻辑难点：**
“临界点 $I_{sat}$ 正好受电动势限制”的措辞较复杂，会让学生在 $I^2 R$ 与 $T^4$ 的替换中迷失。评分细则中需明确演算路径，指出如何在此条件下消除 $I$ 或得到材料常数主导的表达式。
...
- **核心符号：** $I$, $\vec{F}_L$, $\vec{v}$, $P_{rad}$, 阴极, 阳极。
...
def generate_figure_2():
    ...
    ax.text(0.3, 2, '上端卫星（阳极）', va='center')
    ax.text(0.3, -2, '下端卫星（阴极）', va='center')
